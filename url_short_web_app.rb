...
class UrlShortWebApp
  ...
  def create_slug(req, params)
    ...
    [201, {"Content-Type" => "application/json"}, [{ short_url: slug, url: @repo.get(slug) }.to_json] ]
  end

  def use_slug(req, params)
    url = @repo.get(req.request_path)
    if url.nil?
      response_404
    else
  ...
  def index_slugs(req, params)
    [200, {"Content-Type" => "application/json"}, [{ urls: @repo.all }.to_json] ]
  end
